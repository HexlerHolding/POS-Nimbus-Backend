...
const cashierController = {
  ...
  // Updated addOrder function for cashierController// Updated addOrder function for cashierController
  ...
  try {
    ...
    if (customer_phone && customer_phone.trim() !== '') {
      try {
        ...
        // For hello_world template - not passing customer name as hello_world doesn't use parameters
        const notificationResult = await whatsappNotifier.sendWhatsAppMessage(
          customer_phone,
          null, // No parameters needed for hello_world template
          'hello_world' 
        );
 ...
 completeOrder: async (req, res) => {
  try {
    ...
    if (order.order_type === 'delivery' && order.customer_phone && order.address !== 'In Branch') {
      try {
        await whatsappNotifier.sendWhatsAppMessage(
          order.customer_phone,
          whatsappNotifier.messages.orderReady(order.customer_name)
        );
      } catch (notificationError) {
        console.error('Failed to send WhatsApp notification:', notificationError);
        // Continue with the order process even if notification fails
  ...
  try {
    ...
    if (order.order_type === 'delivery' && order.customer_phone && order.address !== 'In Branch') {
      try {
        await whatsappNotifier.sendWhatsAppMessage(
          order.customer_phone,
          whatsappNotifier.messages.orderCancelled(order.customer_name)
        );
      } catch (notificationError) {
        console.error('Failed to send WhatsApp notification:', notificationError);
        // Continue with the order process even if notification fails
